...
class client:
    ...
    def headerconstructor(self,image_url,text:str =None,name: str=None) -> dict:
        if name != None and text != None:
            postdict = {'token': self.token, 'url': image_url,'name':name,'text':text}
        elif text != None:
    ...
    async def gif(self,feature:str,image_url:str):
        feature_list = ['paint','solar','blur','invert','pixel','sepia','wasted','gay','charcoal']
        if feature not in feature_list:
            ...
            response = await self.httpclient.post(url,header)
            return (response)
    async def usertextimage(self,feature:str,image_url:str,text:str, name:str = None):
